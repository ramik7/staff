#!/pkg/marvell/software/perl/5.26.1/bin/perl

#Rami Krankurs
#January 2018

$ENV{PATH}="/bin:/usr/bin:/sbin:/usr/sbin:/usr/local/bin:/usr/local/sbin:/uge/ncd/current/bin/lx-amd64";
use Env::Path;
#
...
#
$SGE_ROOT="/uge/ncd/current";
$ARCH="$SGE_ROOT/util/arch";
$DEFAULTMANPATH="$SGE_ROOT/util/arch -m";
$MANTYPE="$SGE_ROOT/util/arch -mt";
$SGE_CELL="misl_ncd";
$SGE_CLUSTER_NAME="misl6460";
$SGE_QMASTER_PORT="6460";
$SGE_EXECD_PORT="6461";
$SGE_PATH="${SGE_ROOT}/bin/${ARCH}";

Env::Path->PATH->Append("${SGE_ROOT}/bin/${ARCH}");
#==============================================================
#my $nop = @ARGV;
#if("$nop" == "0"){&print_usage;}
#if (("$ARGV[0]" ne "--sge")||("$nop" != "2")){&print_usage;}

#sub print_usage{
#	my $usage="$0 --sge <flow_namea>";
#	print "Usage: $usage\n";
#	exit;
#}
#==============================================================
